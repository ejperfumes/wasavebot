        ...
        {list.map((item) => {
          ...
          return (
            ...
            >
              ...
              <div className="truncate p-2 text-xs font-medium">{item.name}</div>
            </button>
          );
        })}
      </div>
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-3xl">
        ...
        <Tabs value={tab} onValueChange={(v) => setTab(v as MediaItem["type"])}>
          ...
          <TabsContent value="image" className="mt-4 max-h-[55vh] overflow-y-auto">
            {renderGrid(grouped.image)}
          </TabsContent>
          <TabsContent value="video" className="mt-4 max-h-[55vh] overflow-y-auto">
            {renderGrid(grouped.video)}
          </TabsContent>
          <TabsContent value="audio" className="mt-4 max-h-[55vh] overflow-y-auto">
            {renderGrid(grouped.audio)}
          </TabsContent>
          <TabsContent value="document" className="mt-4 max-h-[55vh] overflow-y-auto">
            {renderGrid(grouped.document)}
          </TabsContent>
        </Tabs>
